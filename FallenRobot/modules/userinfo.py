...
async def group_info(_, message: Message) -> None:
    if message.from_user.id not in (TIGERS or []) + (DRAGONS or []) + (DEMONS or []):
        return
    args = message.text.split(" ", 1)
    if len(args) < 2:
        await message.reply("Provide a chat username or ID.")
        return
    chat_query = args[1]
    try:
        chat = await pbot.get_chat(chat_query)
        totallist = []
        async for member in pbot.get_chat_members(chat.id, filter=ChatMembersFilter.ADMINISTRATORS):
            totallist.append(member)
    except Exception:
        await message.reply(
    ...
    msg = f"**ɪᴅ**: `{chat.id}`"
    msg += f"\n**ᴛɪᴛʟᴇ**: `{chat.title}`"
    if hasattr(chat, 'dc_id') and chat.dc_id:
        msg += f"\n**ᴅᴄ**: `{chat.dc_id}`"
    msg += f"\n**sᴜᴩᴇʀɢʀᴏᴜᴩ**: `{chat.type.value}`"
    if chat.username:
        msg += f"\n**ᴜsᴇʀɴᴀᴍᴇ**: {chat.username}"
    msg += f"\n**ᴍᴇᴍʙᴇʀs**: `{chat.members_count}`"
    msg += f"\n**ᴀᴅᴍɪɴs**: `{len(totallist)}`"
    msg += "\n\n**ᴀᴅᴍɪɴs ʟɪsᴛ:**"
    for member in totallist:
        msg += f"\n• [{member.user.id}](tg://user?id={member.user.id})"
    if chat.description:
        msg += f"\n\n**ᴅᴇsᴄʀɪᴩᴛɪᴏɴ**:\n`{chat.description}`"
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "Hᴇᴀʟᴛʜ",
                                url="https://graph.org/Rem-has-bot-access-levels-we-call-as-Disaster-Levels-11-10",
                            ),
                            InlineKeyboardButton(
                                "Dɪꜱᴀꜱᴛᴇʀ",
                                url="https://graph.org/Rem-has-bot-access-levels-we-call-as-Disaster-Levels-11-10-2",
                            ),
                        ],
                    ]
                ),
